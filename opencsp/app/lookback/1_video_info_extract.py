...
class LookBack_ExEx:
    ...
    def extract_frames_of_interest(self):
        frame_names_exts = it.image_files_in_directory(os.path.join(self.current_video_dir, self.output_dir_struct[2]))
        for i, name in enumerate(frame_names_exts):
            if i < self.start_frame:
                continue
            elif i >= self.start_frame and i <= self.end_frame:
                shutil.copyfile(
                    os.path.join(self.current_video_dir, self.output_dir_struct[2], name),
